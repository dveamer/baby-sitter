...
- idle web preview JPEG 생성은 이미 빠졌으므로, 무시청 상태에서도 남는 열은 recorder 비용 비중이 더 클 가능성이 있다.
...
- `01`, `02`가 먼저 반영된 만큼, 여기서는 실제 웹 시청자가 없는 상태에서도 남는 발열의 더 큰 비중이 recorder 비용인지 확인한 뒤 들어가는 편이 좋다.
...
- `01`, `02`, `03`, `06` 이후에도 발열이 충분히 남을 때만 적용하는 마지막 단계로 남는다.
