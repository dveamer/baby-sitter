...
- `02-on-demand-web-preview.md`도 반영되어, active subscriber가 없을 때는 preview JPEG 생성이 중단된다.
- `07-sleep-off-input-gating.md`도 반영되어, `sleepEnabled=false`이고 viewer가 없을 때는 camera/audio collect input 자체가 열리지 않는다.
...
- idle web preview JPEG 생성은 이미 빠졌고, `sleep off + viewer 없음` collect도 `07`에서 꺼지므로, 이후에도 남는 열은 주로 `sleep on` 또는 active preview subscriber 상태의 recorder 비용 비중이 더 클 가능성이 있다.
...
- `01`, `02`, `07`이 먼저 반영된 만큼, 여기서는 `sleep on` 또는 실제 웹 시청 중 상태에서도 남는 발열의 더 큰 비중이 recorder 비용인지 확인한 뒤 들어가는 편이 좋다.
...
- `01`, `02`, `03`, `06`, `07` 이후에도 발열이 충분히 남을 때만 적용하는 마지막 단계로 남는다.
