...
const results = JSON.parse(fs.readFileSync("naver_member_levels.json", "utf8"));
let md = fs.readFileSync(mdPath, "utf8");

function summarize(second) {
  if (!second) return "원문 확인 결과 두 번째 등급 행을 찾지 못함.";

  const desc = second.description ? `원문 설명: ${second.description}. ` : "";
  if (!second.condition) {
    return `\`${second.level}\`: ${desc}원문상 별도 자동/신청 수치 조건은 없음.`;
  }

  let condition = second.condition
    .replace(/\s+/g, " ")
...
md = lines.join("\n");
md = md.replace(
  /- 2026-04-30: 총 40개 수집, 5개는 두 번째 등급 기준 확인, 35개는 원문 확인 필요\.\n(?:  - .+\n){3}/,
  "- 2026-04-30: 총 40개 수집, 40개는 두 번째 등급 기준 확인.\n" +
    "  - 이번 실행(openChrome 원문 재확인): 기존 `원문 확인 필요` 35개를 네이버 `member-level` 원문 DOM 테이블에서 직접 확인해 두 번째 등급 기준으로 갱신.\n" +
